...
def gettime():
    ...
    tservert=ss('time')
    print str(tservert)
    temp=list(str(tservert))
    tservert=''
    for i in range(19):
        tservert+=temp[i]
    print tservert
    servert=time.mktime(time.strptime(tservert,'%Y-%m-%d %H:%M:%S'))
    print '系统时间：',servert
#判断
    if tset>servert:
...
if not os.path.isdir(new_path):
    ...
    f.write('越南测试服'+':'+'49.213.70.62'+';'+'1003'+'\n')
    f.write('泰国测试服'+':'+'202.60.196.191'+';'+'1003'+'\n')
    f.write('台湾测试服'+':'+'210.61.161.5'+';'+'1003'+'\n')
    f.write('韩国测试服'+':'+'118.130.49.15'+';'+'1003'+'\n')
    f.write('版署测试服'+':'+'125.90.196.204'+';'+'1003'+'\n')
    f.close()
...
#上传脚本
def selectbendi():
    global filenamebendi
    filenamebendi = tkFileDialog.askopenfilename(initialdir = 'C:\Users\Administrator\Desktop')
    tbendi.insert(INSERT,filenamebendi)

def selectserv():
    global filenameserver
    catadata=ss('cata')
    catadata=catadata.split('&')
    servname=lb.get(lb.curselection())
    c1='C:\Users\Administrator\Documents\Server Control'
    c2=c1
    c2+=catadata[0].split('\\')[0]
    if not os.path.isdir(c1):
         os.makedirs(c1)
    c=c1
    for i in range(len(catadata)):
        c1=c
        catadata[i]=catadata[i].replace('/','\\')
        c1+=catadata[i]
        if not os.path.isdir(c1):
            os.makedirs(c1)
    filenameserver = tkFileDialog.askdirectory(initialdir = '%s'%c2).split('C:/Users/Administrator/Documents/Server Control')[1]
    tserver.insert(INSERT,filenameserver)
    shutil.rmtree(c2,True)
...
#    ip1 = string(ip)
    ...
    print ip1
    localpath='R\''
    localpath+=filenamebendi
    localpath+='\''
    print localpath

    temp=localpath.split('/')


    t=paramiko.Transport(('118.130.49.15',port))
    t.connect(username='program',password='jojwor?2343joljo')
    sftp=paramiko.SFTPClient.from_transport(t)
    #stdin,stdout,stderr = ssh.exec_command('ls')

    remotepath=filenameserver
    remotepath+='/'
    remotepath+=temp[len(temp)-1].split('\'')[0]
#    print remotepath
 #   print remotepath,"rrrrrrrrrr"
    sftp.put(filenamebendi,remotepath)
    t.close()
...
def upload():
    ...
    global top2
    top2=Toplevel()
    top2.title('UploadFile')
    top2.geometry('400x125+705+240')
    #上传界面
    ftop2=Frame(top2,height='125',width='400',bg='aliceblue')
    ftop2.pack()
    lawarning2=Label(top2,text='Warning:请先确定自己拥有更改文件的权限',fg='red',bg='aliceblue',width=50)
    lawarning2.place(x=20,y=0)
    labendi=Label(top2,text='本地文件：',bg='aliceblue')
    labendi.place(x=0,y=30)

    global tbendi,tserver
    tbendi=Text(top2,width=40,height=2,font=ft)
    tbendi.place(x=75,y=28)
    btbendi=Button(top2,text='选择文件',command=lambda:selectbendi())
    btbendi.place(x=325,y=25)
    laserver=Label(top2,text='服务器路径：',bg='aliceblue')
    laserver.place(x=0,y=65)
    tserver=Text(top2,width=40,height=2,font=ft)
    tserver.place(x=75,y=65)
    btserv=Button(top2,text='选择目录',command=lambda:selectserv())
    btserv.place(x=325,y=65)
    btok2=Button(top2,text='上传',width=10,command=lambda:sendm())
    btok2.place(x=160,y=93)
...
#上传脚本
btupd=Button(root,text='上传文件',fg='dodgerblue',bg='ivory',activebackground='lightblue',height='1',width='10',command=upload)
btupd.place(x=230,y=25)
#更新脚本
laupd=Label(root,text='服务器文件上传',bg='aliceblue')
...
n=0
ft = tkFont.Font(family = 'Fixdsys',size = 9)
root.mainloop()
